...
class WebsiteTasks(TaskSet):
    ...
    def jiayoukaRecharge(self):
        ...
        with self.client.get(
            ...
            if response.json()['code'] != 0:
                response.failure("main_info接口，code不为0:{}".format(response.json()))
        # 绑定用户
        with self.client.get(
        ...
        with self.client.post(
            ...
            if re.search('"id":"(.*)","name"', text) != None:
                ...
                with self.client.get("/api/order/v1/lite/{0}?id={0}&subId=&orderType=jiayouka".format(id),
                    ...
                    if response.json()['code'] != 0:
                        response.failure("订单详情{}，code不为0:{}".format(id, response.json()))
    ...
    def getOrderList(self):
        ...
        with self.client.get(
            ...
            if response.json()['code'] != 0:
                response.failure("订单列表，code不为0:{}".format(response.json()))
